...
<template>
    <div class="p-6">
        ...
        <div class="mb-6 flex items-center justify-between">
            <h1 class="text-2xl font-bold text-gray-800">إدارة الخدمات والأسعار</h1>
            <div class="flex gap-3">
                ...
                <button class="btn-primary" @click="openCreate">+ إضافة خدمة</button>
            </div>
        </div>
        ...
        <div class="mb-4 flex flex-wrap gap-3">
            ...
            <select v-model="filters.status" class="input-field" @change="search">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="svc in services.data" :key="svc.id" class="border-t border-gray-100 hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium">{{ svc.name }}</td>
                        <td class="px-4 py-3">
                            <Badge variant="active">{{ deptLabels[svc.dept] || svc.dept }}</Badge>
                        </td>
                        <td class="px-4 py-3">{{ svc.price.toFixed(2) }} ج</td>
                        <td class="px-4 py-3 text-blue-600">{{ svc.ins_price.toFixed(2) }} ج</td>
                        ...
                        <td class="px-4 py-3">
                            <Badge :variant="(svc.status === 'active' ? 'active' : 'cancelled')">
                                {{ svc.status === 'active' ? 'نشط' : 'غير نشط' }}
                            </Badge>
                        </td>
                        <td class="px-4 py-3">
                            <button class="text-gray-400 hover:text-blue-600" @click="openEdit(svc)">
                                <Edit2 class="h-4 w-4" />
                            </button>
                        </td>
                    </tr>
                    <tr v-if="services.data.length === 0">
                        <td class="px-4 py-8 text-center text-gray-400" colspan="8">لا توجد خدمات</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <!-- Create/Edit Modal -->
        <Modal :show="showModal" :title="editingService ? 'تعديل خدمة' : 'إضافة خدمة'" @close="showModal = false">
            <form class="space-y-4" @submit.prevent="submit">
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <select v-model="form.dept" class="input-field" required>
                            <option v-for="(label, key) in deptLabels" :key="key" :value="key">{{ label }}</option>
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model.number="form.ins_price" class="input-field" type="number" min="0" step="0.01" />
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input v-model.number="form.center_val" class="input-field" type="number" min="0" step="0.01" />
                    </div>
                    <div>
                        <label class="mb-1 block text-sm font-medium">مدة الخدمة (دقيقة)</label>
                        <input v-model.number="form.duration_mins" class="input-field" type="number" min="1" />
                    </div>
                </div>
        ...
        <!-- Import Modal -->
        <Modal :show="showImportModal" title="استيراد الخدمات من Excel" @close="showImportModal = false">
            <form class="space-y-4" @submit.prevent="submitImport">
                <p class="text-sm text-gray-600">
                    الأعمدة المطلوبة: <code>name, dept, price, center_val</code> <br />
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" class="btn-secondary" @click="showImportModal = false">إلغاء</button>
